 ...
 * 原地测量：真实内容不移动不包裹，仅以 qs-skeleton-measuring 类透明化；
 * 覆盖层 portal 挂载到 body 对齐根容器，任意框架 DOM 所有权完整。
 * 动画样式走容器级 --qs-sk-* 变量，样式单例注入（引用计数）。
 */
...
// 视口增量渲染常量：预取 ±1 屏；上限 500，超限淘汰视口外块
...
// 单例样式注入：内容静态，多实例共享同一 style 节点，最后一个销毁时移除
...
export class AutoSkeleton {
  ...
  // 加载期位置守卫：rAF 每帧比对根容器文档坐标，变化即重定位 + 重测；
  // 纯滚动时文档坐标恒定（rect 与 scrollY 抵消），ResizeObserver 管不到布局沉降
  ...
  /** 覆盖层 DOM（portal 挂载于 body）；可用于自定义退出动画 */
   ...
   * 纯滚动坐标恒定 → 零操作；布局沉降/内容重排 → 重定位 + 重测（节流 100ms）。
   * 补 ResizeObserver 只报尺寸变化的盲区。
   */
  ...
  /** 滚动增量补渲（append-only）：补渲新进入 ±1 屏的块，超上限淘汰视口外块（DOM 有界） */
  ...
  private setupObservers(): void {
    ...
    // 滚动跟随：捕获阶段收到任意滚动容器滚动，rAF 合并做增量补渲（独立 rAF 不互吞）
    this.scrollHandler = () => {
      if (this.isDestroyed || !this.options.loading) return;

      // 防御：根容器脱离文档（未 destroy 就卸载）→ 自毁，避免覆盖层钉在左上角漂浮
    ...
    // refetch 自适应：结构变化 → rAF 合并 → 签名比对 → 变了才重测；
    // 文本只比长度，宁可错报不可漏报

...
Or clone the repo (qingwu-ui monorepo, in `packages/editor`):
...
```ts
import { startClipperReceiver } from "@qingwu/editor";
...
| `maxAttachmentSize` | `number` | **required** | Max size of a single uploaded file (bytes); oversize files are blocked with a toast |

/** 青梧UI · 标签快捷插入组件：输入框 + 快捷标签栏，受控/非受控，零框架依赖（纯 DOM + CSS） */

import type { ChipItem } from "@qingwu-ui/text-layout";
import { layoutChips } from "@qingwu-ui/text-layout";
...
export class TagInput {
  ...
  /** 插入标签到输入框（已存在则忽略）；非受控立即生效，受控触发 onChange 等外部同步 */
  ...
  /** 将文本创建为新标签加入快捷栏（allowEnterCreate 内部逻辑，也可程序化调用）；已存在则忽略并清空输入 */

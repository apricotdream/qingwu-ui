...
标签快捷插入组件：输入框 + 下方标签快捷栏。点击标签按钮自动填入输入框；已插入的标签从快捷栏消失，从输入值中删除后自动重现。布局由 `@apricotdream/text-layout` 的 `layoutChips` 驱动（展开/收起 + 标签栏高度）。
...
```bash
npm install @apricotdream/tag-input
```

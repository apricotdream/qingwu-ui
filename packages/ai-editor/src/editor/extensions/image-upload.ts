...
async function insertMdFile(view: any, file: File, pos?: number, editor?: any) {
  const insertPos = pos ?? view.state.selection.from;
  // 限制实时从 qingwuUI storage 读取（宿主运行期可更新）
  const limits = editor ? getEditorAttachmentLimits(editor) : undefined;
...
async function uploadAndInsert(view: any, file: File, pos?: number, editor?: any) {
  // 同步校验：超限直接拒绝，不插入占位节点（限制实时从 qingwuUI storage 读取）
  const limits = editor ? getEditorAttachmentLimits(editor) : undefined;
  const limitErr = limits ? validateAttachmentFile(view.state.doc, file, limits) : null;
  if (limitErr) {
    toast(limitErr);
...
export const ImageUpload = Extension.create<AttachmentLimits>({
  ...
  // 默认 0 = 不限制；QingWuAIEditor 总是通过 configure 传入真实限制
  addOptions() {
        ...
        props: {
          handleDrop(view, event) {
            ...
            for (const file of Array.from(files)) {
              if (isMdFile(file)) {
                insertMdFile(view, file, pos?.pos, editor);
              } else {
                uploadAndInsert(view, file, pos?.pos, editor);
              }
            }
            return true;
          },
            ...
            for (const item of Array.from(items)) {
              if (
                ...
                if (file && detectType(file) !== "file") {
                  uploadAndInsert(view, file, undefined, editor);
                }
                return true;
              }
              // 粘贴 MD 文件 → 直接渲染
              if (item.kind === "file") {
                ...
                if (file && isMdFile(file)) {
                  event.preventDefault();
                  insertMdFile(view, file, undefined, editor);
                  return true;
                }
              }
            }

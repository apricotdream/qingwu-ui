...
function insertNode(
  editor: Editor,
  node: ReturnType<Editor["schema"]["nodes"]["paragraph"]["create"]>,
) {
  act(() => {
...
describe("getDocAttachmentTotal", () => {
  ...
  it("统计 attachment/video/audio/image 节点的 size 之和", async () => {
    ...
    insertNode(
      editor,
      attachmentEmbed.create({ src: "blob:a", name: "a", size: 1 * MB, type: "" }),
    );
    insertNode(
      editor,
      videoEmbed.create({ src: "blob:v", source: "direct", name: "v", size: 2 * MB }),
    );
...
describe("validateAttachmentFile", () => {
  ...
  it("单文件与总大小同时超限 → 只报单文件错误，不合并", async () => {
    ...
    expect(validateAttachmentFile(editor.state.doc, fakeFile(60 * MB), LIMITS)).not.toMatch(
      /总大小/,
    );
  });

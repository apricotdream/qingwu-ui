...
> **包名说明：** 对外发布的 npm 包名是 scoped 名 **`@qingwu/ai-editor`**，`npm install` 与所有 `import` 都用它（含样式子路径 `@qingwu/ai-editor/styles`、Node 子入口 `@qingwu/ai-editor/clipper`）。包名历史：`qingwu-ai-editor`（unscoped）→ `@qingwu/editor` → **`@qingwu/ai-editor`**（现名）；若你在旧文档/示例里看到前两者，请替换为 `@qingwu/ai-editor`。
...
import "@qingwu/ai-editor/styles";
...
> **TypeScript 类型：** 本包已为样式子路径提供类型声明（`styles.d.ts`，经 `exports` 的 `./styles` 暴露），`import "@qingwu/ai-editor/styles"` 通常**无需**额外配置即可通过类型检查。若你的 TS 仍报 `2307 找不到模块`，可在项目任意 `.d.ts` 补一行兜底，**不要**因此删掉样式 `import`：
> ```ts
...
const Editor = dynamic(
  () => import("@qingwu/ai-editor").then((m) => m.QingWuAIEditor),
  { ssr: false }
);
...
> **Node / 桌面壳专用子入口：** `startClipperReceiver` / `stopClipperReceiver` 依赖 `node:http`，已从浏览器主入口移除，请从 `@qingwu/ai-editor/clipper` 导入，避免把 Node 模块打进浏览器 / SSR 客户端产物。纯浏览器场景仍从主入口用 `startBrowserClipperReceiver`（见下文）。

```ts
import { startClipperReceiver } from "@qingwu/ai-editor/clipper";

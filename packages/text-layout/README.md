...
- **Chip 流式布局**：`layoutChips` 驱动标签流（展开 / 收起 + 高度计算，`@apricotdream/tag-input` 即基于它）
- **表格列宽**：`computeColumnWidths` / `fitRowToColumns` 自适应列宽计算

## 安装

```bash
npm install @apricotdream/text-layout
```

## 使用

```ts
import { layout, measure, prepare } from "@apricotdream/text-layout";

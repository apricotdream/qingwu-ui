...
function getLunarYearMeta(lunarYear: number): LunarYearMeta {
  ...
  if (idx < 0 || idx >= LUNAR_INFO.length) {
    throw new RangeError(`农历年份 ${lunarYear} 超出支持范围（${BASE_YEAR}-${END_YEAR - 1}）`);
  }
...
function _toMonthIndex(meta: LunarYearMeta, lunarMonth: number, isLeap: boolean): _MonthIndex {
  ...
  if (isLeap) {
    if (lunarMonth !== leapMonth) {
      throw new RangeError(`${lunarMonth} 月不是闰月，该年闰月为 ${leapMonth} 月`);
    }
...
function _fromMonthIndex(
  meta: LunarYearMeta,
  idx: number,
): { displayMonth: number; isLeap: boolean } {
  const { leapMonth } = meta;

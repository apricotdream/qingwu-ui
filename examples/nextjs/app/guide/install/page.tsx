...
const PACKAGES = [
  { name: "@qingwu/calendar", desc: "日历 + 农历引擎：农历 / 节气 / 节日 / 黄历宜忌 / 休假表 / 禁用规则" },
  ...
  return (
    <article className="docs-article">
      ...
      <p>
        ...
        所有包均零运行时依赖、纯 TypeScript 编写、ESM / CJS 双格式输出，React / Vue / 原生 HTML 通用。
      </p>
      <table>
        <thead>
          <tr><th>包名</th><th>说明</th></tr>
        </thead>
        <tbody>
          {PACKAGES.map((p) => (
            <tr key={p.name}>
...
# bun
bun add @qingwu/calendar`}</code></pre>

      <h2 id="usage">快速使用</h2>
      <p>组件是<b>框架无关</b>的：实例化时传入挂载节点与配置即可。以日历为例：</p>
      <pre><code>{`import { Calendar } from "@qingwu/calendar";
...
// 卸载时释放资源
cal.destroy();`}</code></pre>

      <p>在 React 中，通过 <code>useEffect</code> 管理生命周期：</p>
...
function CalendarPicker() {
  ...
  return <div ref={rootRef} />;
      ...
      <h2 id="cdn">CDN 引入</h2>
      <p>无需构建工具，直接通过 <code>unpkg</code> 在浏览器使用：</p>
      <pre><code>{`<script type="module">
  import { Calendar } from "https://unpkg.com/@qingwu/calendar";
</script>
<link rel="stylesheet" href="https://unpkg.com/@qingwu/calendar/style.css" />`}</code></pre>

      <h2 id="support">浏览器支持</h2>
      <p>
        所有组件基于现代浏览器标准（原生 DOM、CSS 自定义属性、ES2020+），
        不兼容 IE。推荐使用最新版 Chrome / Edge / Firefox / Safari。
      </p>
    </article>
  );
}

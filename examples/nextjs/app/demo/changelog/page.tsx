...
const VERSIONS = [
  ...
  },
  {
    ...
    tag: "Toast 组件",
    changes: [
      { type: "feat", text: "新增 @qingwu/toast 轻提示组件：零依赖、纯 TypeScript + 原生 DOM 渲染" },
      { type: "feat", text: "ARIA live region 内建（role=status + aria-live=polite），prefers-reduced-motion 自动克制" },
      { type: "feat", text: "6 种定位（top/bottom × left/center/right），4 种语义类型（info/success/warning/error）" },
      { type: "feat", text: "Promise 链：loading → success/error 三态自动流转，支持函数式消息生成" },
      { type: "feat", text: "队列管理：maxVisible 控制同时显示上限，超出自动排队，关闭后按序出队" },
      { type: "feat", text: "现代 UI 设计：毛玻璃质感卡片、实心彩色图标圆底、双层柔光阴影、弹簧曲线入场动画" },
      { type: "feat", text: "移动端适配：安全区 inset 偏移、窄屏自适应宽度与两行文本换行、44px 触控热区" },
      { type: "feat", text: "明暗双主题：自动响应用户主题偏好，CSS 自定义属性驱动，支持宿主覆盖" },
      { type: "feat", text: "演示页：语义类型卡片、3×2 定位可视化网格、5 种场景演示、操作日志" },
      { type: "improve", text: "优化 Toast 动画为纯 opacity 淡入淡出，不触发 GPU 合成层，多 toast 并发无显存膨胀" },
  ...
  },
  {
    ...
    tag: "工程加固",
    changes: [
      { type: "improve", text: "发版流程接入 publish-check 产物校验门禁：workspace 依赖残留检测 / CHANGELOG 版本一致性 / exports 产物齐全性" },
      { type: "improve", text: "新增 Playwright e2e 端到端测试：拖拽上传、压缩产出 WebP/AVIF、单张数量限制、按钮触发、真实 HTTP 上传" },
  ...
  },
  {
    ...
    tag: "Upload 上传",
    changes: [
      { type: "feat", text: "新增 @qingwu/upload 图片上传组件：拖拽区 / 按钮两种触发形态，按钮形态复用 @qingwu/button" },
      { type: "feat", text: "客户端压缩管线：支持原图 / WebP / AVIF 多份并行输出，AVIF 不支持时自动降级 WebP/PNG" },
      { type: "feat", text: "独立进度条组件，内置 XHR 上传与可插拔自定义上传函数，支持数量/大小/类型校验" },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "feat", text: "@qingwu/button 按钮：胶囊形（pill）风格，default / primary / amber / icon 四种变体，纯 DOM + CSS" },
      { type: "feat", text: "@qingwu/editor 编辑器：Tiptap/ProseMirror 内核，斜杠命令、AI 写作助手（OpenAI/DeepSeek/Qwen）、代码高亮、i18n、Web Clipper" },
      { type: "feat", text: "@qingwu/search 搜索：打字机占位轮播、Ctrl/⌘+K 全局唤起、全键盘导航+焦点陷阱、ARIA 完整语义、分类筛选" },
      { type: "feat", text: "@qingwu/calendar-core 日历引擎：headless 纯日期工具，零依赖零 DOM 副作用，tree-shakeable" },
    ],
  },
  {
    version: "0.1.0",
    ...
    tag: "内部先行",
    changes: [
      { type: "feat", text: "发布 @qingwu/calendar-core@0.1.0 与 @qingwu/search@0.1.0（私有 Nexus registry）" },
      { type: "feat", text: "Calendar 弹出日历组件：农历/节气/节日/黄历宜忌详情侧栏，日期/月份/年份三视图，键盘导航" },
      { type: "feat", text: "自研农历引擎 lunar.ts：零依赖，覆盖 1900-2100 年公农历互转、节气（基础日期+年份修正）、天干地支" },
      { type: "feat", text: "休假日历 JSON 配置：holidays/workdays 支持，日期格右上角「休/工」角标" },
      { type: "feat", text: "演示页 props 面板：属性实时修改 + 应用配置重新渲染，展开代码支持 React / HTML / Vue 三格式" },
      { type: "fix", text: "修复公农历转换中闰月定位算法错误（原实现存在边界 bug）" },
      { type: "fix", text: "节气数据从固定日期升级为基础日期 + 年份修正表（2020-2030 精度 ±0 天）" },
  ...
  return (
    <div className="changelog">
      ...
      <div style={{ marginTop: 28, fontSize: 13, color: "var(--ink-3)" }}>
        ...
        <Link href="/" className="home-card-link" style={{ color: "var(--teal)", textDecoration: "underline" }}>
          回到首页
        </Link>
      </div>
    </div>

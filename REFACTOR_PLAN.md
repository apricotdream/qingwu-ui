...
> 版本：v1.1（2026-07-31 修订：`@apricotdream/calendar-core` 并入 `@apricotdream/calendar`，状态引擎不再独立成包）｜ 编制日期：2026-07-25 ｜ 性质：规划文档（不含任何代码改动）
...
- **一次逻辑，一套原生渲染**：状态引擎与渲染层同置于 `@apricotdream/calendar` 包内（纯逻辑模块零 DOM 依赖、Node 可测），原生 DOM 渲染层为正统，React/Vue 仅为生命周期薄包装——测试与 bug 修复只做一遍，框架大版本升级近乎无感（决策见 §6.6）。
...
- `@apricotdream/calendar`（**产品主体**：内置 headless 状态引擎 + 原生 DOM 渲染层 + 默认样式）、`@apricotdream/calendar-react` / `@apricotdream/calendar-vue`（生命周期薄包装，各 ~100 行）。
- `@apricotdream/lunar`（历法数据包）、`@apricotdream/icons`（样式 tokens 并入 `@apricotdream/calendar` 的 CSS 子路径导出，不单设包）。
- 文档站 + playground + 迁移指南 + 旧 API 兼容层 `@apricotdream/calendar-compat`。
...
│   │   ├── calendar/         # @apricotdream/calendar —— 产品主体：内置 headless 状态引擎 + 原生 DOM 渲染层 + 默认样式
│   │   │   └── src/
...
│   │   ├── react/            # @apricotdream/calendar-react —— ~100 行生命周期包装（非渲染层重写）
│   │   ├── vue/              # @apricotdream/calendar-vue —— ~80 行生命周期包装 + v-model 桥
...
### 6.2 核心引擎设计（@apricotdream/calendar 内置模块）
...
### 6.3 样式架构（`@apricotdream/calendar` 样式子系统，`./css` 与 `./themes/*` 子路径导出）
...
- 弹层定位：优先 CSS Anchor Positioning，不支持的浏览器降级为 JS 定位（@apricotdream/calendar 内 positioning 模块）。
...
| CDN bundle | 仅 @apricotdream/calendar 产出单文件 IIFE（给无构建环境，替代旧 UMD） |
...
- 体积卡口：**@apricotdream/calendar 主体 gzip ≤ 30KB**、lunar 子路径 gzip ≤ 30KB、CI `size-limit` 超限即红。
...
- **背景**：原方案为独立 core 状态引擎 + DOM/React/Vue 三套渲染层。评审后改判；2026-07-31 再修订：状态引擎并入 @apricotdream/calendar 包内模块（原独立 calendar-core 包仅 44 行且无人实质依赖，判定独立成包无必要）。
- **决策**：**原生 TS 实现为主体**——`@apricotdream/calendar` 拥有唯一的正统 DOM 渲染层；React/Vue 官方包是 ~100 行的生命周期薄包装（mount/update/destroy + 事件转发），不含任何渲染逻辑；Web Component 封装作为后续 bonus。
- **理由**：
...
- 交付物：`@apricotdream/calendar@0.1.0` 内置状态引擎（包内纯逻辑模块，零 DOM 依赖，Node 环境可跑全部测试）。

### Phase 3 ｜ 产品主体：原生渲染层 + 样式体系（第 8–10 周）★ 渲染层正统在此

- `@apricotdream/calendar` render 模块：内置 ViewState → DOM 渲染（节点复用代替全量重建，解决 §1.6 重绘问题）；Popover API + Anchor Positioning（降级方案）；SSR 注水就绪；`new QingwuCalendar(el, opts)` 命令式 API（与主流日历库 API 同构，为 compat 层铺路）。
...
- `@apricotdream/lunar`：基于自研 lunar.ts 封装，子路径拆分（`/core` 农历日/节日/节气，`/almanac` 宜忌，`/eightchar` 八字）；lunar 插件实现 @apricotdream/calendar 内置 `DayMetaProvider` 接口。
...
- `@apricotdream/calendar-react`（~100 行）：useEffect 创建/destroy + props 转发 + 受控事件；React Hook Form 友好。
- `@apricotdream/calendar-vue`（~80 行）：onMounted/onUnmounted + v-model 桥 + slot 转发。
- 包装层零渲染逻辑 → 无行为漂移问题；测试 = 挂载冒烟 + 事件转发断言，不再跑三端行为矩阵。

### Phase 6 ｜ 兼容层、文档与首发（第 15–16 周）

- `@apricotdream/calendar-compat`：旧库 `(el, options)` 签名兼容 + 运行时 deprecation 提示，覆盖 80% 高频 options 映射（原生 API 同构，此层仅为参数映射函数集）。
...
**产物质量**
- @apricotdream/calendar 主体 gzip ≤ 30KB；lunar 子路径 gzip ≤ 30KB；状态/规则模块测试覆盖 ≥ 90%；框架包装层合计 < 300 行源码。
...
| 架构一句话？ | 原生 TS 为主体（@apricotdream/calendar 内置 headless 状态引擎 + 原生 DOM 正统渲染）+ React/Vue 百行薄包装 + token/@layer 样式 + 农历 dayMeta 管线（§6.6） |
